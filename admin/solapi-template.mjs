...
/* ---------------------------------------------------------------
   ...
   --------------------------------------------------------------- */
const DRAFTS = {
  /* 신청서를 내면 그 자리에서 주문 확인과 결제까지 이어집니다.
     따로 접수만 받아 두는 단계가 없으므로, 이 문안은 "결제하세요"가 아니라
     "다 끝났습니다"를 알리는 자리입니다.

     무료 강의와 유료 강의를 한 템플릿으로 씁니다. 달라지는 건 금액뿐이라
     #{수강료} 에 담아 보냅니다.
       무료 → "무료"
       유료 → "150,000원 결제 완료"

     녹화본은 이 문안 대신 vod_access 가 나갑니다. */
  apply_confirm: {
    name: "크래빗아카데미_신청완료",
    content: [
      "안녕하세요, 원장님! 크래빗팀입니다 🐰",
      ...
      "■ 수강료  #{수강료}",
      "",
      "시작 하루 전에 오시는 길과 준비물을",
      "한 번 더 보내드릴게요.",
      "",
      "그때까지 궁금하신 점은 편하게 연락 주세요.",
      "#{문의처}"
    ].join("\n"),
    buttons: [
      { buttonName: "강의 안내 보기", buttonType: "WL", linkMo: "#{강의링크}", linkPc: "#{강의링크}" }
    ]
  },

  /* 주문 확인 화면까지 갔다가 결제를 끝내지 않고 나가신 분께 보냅니다.
     신청서는 이미 저장돼 있고 결제만 비어 있는 상태입니다. */
  payment_guide: {
    name: "크래빗아카데미_결제안내",
    content: [
      "안녕하세요, 원장님! 크래빗팀입니다 🐰",
      "",
      "#{강의명} 신청서는 잘 받았는데",
      "결제가 끝나지 않은 채로 남아 있어요.",
      "",
      "■ 일시  #{일시}",
      "■ 수강료  #{수강료}",
      "",
      "아래 버튼을 누르시면 결제 화면으로 바로 이어집니다.",
      "#{마감일}까지 결제가 확인되지 않으면",
      "신청이 자동으로 취소됩니다.",
  ...
  /* ------------------------------------------------------------------
     아래는 아카데미 강의가 아니라 티처스 장학카드 주문 안내입니다.
     발송 주체가 달라서 등록할 때 pfId 를 크래빗 본 채널로 골라야 해요.
     아카데미 채널로 등록하면 원장님께 엉뚱한 채널명으로 나갑니다.

     보내는 시점은 결제가 끝난 순간(PAYMENT_COMPLETED)입니다.
     그다음 단계가 전자계약이라 그 한 줄을 반드시 담았어요.
     제작 기간은 최근 120일 158건의 실제 기록에서 가져왔습니다.
     결제부터 수령까지 평균 10.9일, 2주 안에 끝난 건이 78% 라서 "1~2주"로 씁니다.

     광고 표현이 없어야 심사를 통과합니다. 할인, 지금 신청, 특가 같은 말은 넣지 마세요. */
  /* ------------------------------------------------------------------
     아래 둘은 아카데미 강의가 아니라 티처스 장학카드 주문 안내입니다.
     등록할 때 pfId 를 크래빗 본 채널로 골라야 해요.
     아카데미 채널로 등록하면 원장님께 엉뚱한 채널명으로 나갑니다.

     보내는 시점은 결제가 끝난 순간(PAYMENT_COMPLETED)입니다.
     그다음 단계가 전자계약이라 그 한 줄을 반드시 담았어요.
     제작 기간은 최근 120일 158건의 실제 기록에서 가져왔습니다.
     결제부터 수령까지 평균 10.9일, 2주 안에 끝난 건이 78% 입니다.

     첫 주문과 재주문으로 나눴습니다. 최근 120일 240건 기준으로
     첫 주문 143건, 재주문 97건이에요.

     【시안 문장을 조심해야 하는 이유】
     첫 주문은 143건 중 142건이 새 디자인이라 시안 안내를 그대로 써도 됩니다.
     반면 재주문 97건은 기존 디자인 72건과 새 디자인 25건이 섞여 있어요.
     그래서 재주문 문안은 두 경우를 한 문장에 함께 담았습니다.
     알림톡 템플릿은 조건 분기가 안 되니 이렇게 써야 틀린 안내가 안 나갑니다.

     광고 표현이 없어야 심사를 통과합니다. 할인, 지금 신청, 특가 같은 말은 넣지 마세요. */
  order_confirm_first: {
    name: "크래빗_장학카드주문접수_첫주문",
    content: [
      "안녕하세요, #{학원명} 원장님.",
      "",
      "크래빗 장학카드를 처음 주문해 주셔서 감사합니다.",
      "",
      "■ 주문번호  #{주문번호}",
      "■ 주문내역  #{카드유형} #{수량}장",
      "■ 결제금액  #{금액}원",
      "",
      "전자계약서 서명이 아직 완료되지 않으셨다면,",
      "카드 제작이 원활하게 진행될 수 있도록",
      "확인 후 서명 부탁드릴게요.",
      "",
      "서명이 끝나면 카드 디자인 시안을 준비해",
      "순차적으로 안내드립니다.",
      "",
      "처음이라 궁금하신 점이 많으실 거예요.",
      "언제든 편하게 채팅이나 전화로 문의해 주세요.",
      "010-5957-2483"
    ].join("\n"),
    /* 문의처와 버튼 링크는 변수로 두지 않고 고정값으로 박았습니다.
       변수가 적을수록 심사가 단순하고, 버튼 링크에 변수를 쓰면 반려되는 일이 있어요. */
    buttons: [
      { buttonName: "전자계약 하러 가기", buttonType: "WL",
        linkMo: "https://teacher.crabit.co.kr/auth/login",
        linkPc: "https://teacher.crabit.co.kr/auth/login" }
    ]
  },

  order_confirm_repeat: {
    name: "크래빗_장학카드주문접수_재주문",
    content: [
      "안녕하세요, #{학원명} 원장님.",
      "",
      "장학카드를 다시 주문해 주셔서 감사합니다.",
      "",
      "■ 주문번호  #{주문번호}",
      "■ 주문내역  #{카드유형} #{수량}장",
      "■ 결제금액  #{금액}원",
      ...
      "확인 후 서명 부탁드릴게요.",
      "",
      "기존 디자인 그대로시면 바로 제작에 들어가고,",
      "새로 준비하시는 경우에는 시안을 먼저 안내드립니다.",
      "",
      "궁금하신 점이 있으시면 언제든 편하게",
      "채팅이나 전화로 문의해 주세요.",
      "010-5957-2483"
    ].join("\n"),
    buttons: [
