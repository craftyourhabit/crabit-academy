...
- `claude-code-windows.html` — 윈도우에서 클로드 코드 설치하는 방법을 한 단계씩 안내하는 독립 랜딩페이지. 방법 A(데스크탑 앱, 터미널 없이) / 방법 B(PowerShell `irm https://claude.ai/install.ps1 | iex`) 두 경로를 다루고, 실제 claude.ai 스크린샷(`assets/guide/`)과 CSS로 재현한 PowerShell·클로드 코드·데스크탑 앱 화면을 함께 담았습니다. 2026년 7월 기준(채팅·코워크 통합 반영). 설치 명령/요건은 공식 문서 code.claude.com/docs 기준이라, 버전이 바뀌면 갱신 필요.
