...
- 행사 상세(`event.html`): 본문 하단에 상태별 문구의 "카카오톡 채널 추가하기" 콜아웃이 자동 렌더됩니다.
  - 예정 행사 → "행사 알림 받기"
  - 다시보기 준비 중 → "다시보기 오픈 알림 받기"
  - 마감/종료 → "다음 행사 알림 받기"
- 예정 일정(`schedule.html`): 캘린더 아래 "새 일정 알림 받기" 배너
- 채널 주소는 `assets/events.js` 상단의 `KAKAO_CHANNEL_URL` / `KAKAO_CHANNEL_ADD_URL`에서 관리합니다. (schedule.html 배너와 각 페이지 푸터에는 주소가 직접 적혀 있으니 채널이 바뀌면 함께 수정)
